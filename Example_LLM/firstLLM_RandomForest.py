...
sepal_length = st.slider("Select Sepal Length", min_value=float(df['sepal length (cm)'].min()),
          max_value=float(df['sepal length (cm)'].max()),
            value=float(df['sepal length (cm)'].mean()))
petal_length = st.slider("Select Petal Length", min_value=float(df['petal length (cm)'].min()),
          max_value=float(df['petal length (cm)'].max()),   
            value=float(df['petal length (cm)'].mean()))  
sepal_width = st.slider("Select Sepal Width", min_value=float(df['sepal width (cm)'].min()),
          max_value=float(df['sepal width (cm)'].max()),    
            value=float(df['sepal width (cm)'].mean()))     
petal_width = st.slider("Select Petal Width", min_value=float(df['petal width (cm)'].min()),
